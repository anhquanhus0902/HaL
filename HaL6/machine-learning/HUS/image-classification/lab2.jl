...
# ╔═╡ 4ae6ab1f-362b-43cd-96f5-30b4eb03463f
using Optim
...
# 5. compute the gradient of J(θ): dJ(θ), then use Optim to find θ_best
...
# ╔═╡ 8ed918e3-558f-48c6-8fb2-5c3ac6c9490a
X, y = readData()

# ╔═╡ 90129d71-7331-47ab-bff1-080fad3a05b4
function LSE(θ, X::Array{Float64}) # log-sum-exp function
	u = θ*X
	m = maximum(u)
	v = u-m
	return m+sum(exp.(v))
...
# ╔═╡ 14383397-247d-4174-bc67-e8eeb5920499
function LSE(θ, X::Matrix{Float64})
	U = θ*X'
	m = maximum(U, dims=1)
	K = size(0,1)
	M = repeat(m, K-1)
	v = U-M
	return m + sum(exp.(v), dims=1)
end

# ╔═╡ 7f5ce116-e13a-4a76-b43d-c1106993484c
function l(θ, X, y)
	u = sum(θ[y,:] .* X, dims=2)
	v = LSE(θ, X)
	N = size(X,1)
...
# ╔═╡ ae2b6b35-dadc-4f80-a876-dc490f585f0c
function J(X, y, θ, λ=0)
	l = -l(θ, X, y)
end

# ╔═╡ 35e822ec-69b5-46fc-b621-bd82f090860c
result = optimize(t -> J(X, y, t, λ))

# ╔═╡ 5432a520-ffa1-4c6d-a972-4a63e57cf0b3
