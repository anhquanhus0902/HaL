...
You can use testArray for testing by not entering input and just pressing Enter\n', elements => {
	...
	if (areAllNum) {
		key = Math.floor(Math.random() * (arr.length + 1));
		console.log(arr);
		var res = BinarySearch(arr, arr[key]);
		if (res == -2) {
			console.log('Note that: If you use Binary Search algorithm, your array must be sorted');
			return;
		}
		// Print the result
		console.log(`Index of ${arr[key]} is: ${res}`);
		console.log(key === res);
	}
	readline.close();
});